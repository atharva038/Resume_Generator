...
const TechPortfolioNavbar = ({ data, isDarkMode, toggleDarkMode, accentColor }) => {
  ...
  const navSurface = isDarkMode
    ? scrolled
      ? "border-white/15 bg-slate-950/90 shadow-2xl shadow-black/25"
  ...
  const mutedText = isDarkMode ? "text-slate-400 hover:bg-white/10 hover:text-white" : "text-slate-600 hover:bg-slate-100 hover:text-slate-950";
  const controlText = isDarkMode ? "text-slate-300 hover:bg-white/10" : "text-slate-700 hover:bg-slate-100";
  ...
  return (
    <header className="sticky top-0 z-40 px-3 pt-3 sm:px-6">
      <motion.div
        ...
        className={`mx-auto max-w-6xl rounded-2xl border px-4 py-3 backdrop-blur-xl transition-all sm:rounded-full sm:px-6 ${navSurface}`}
      >
        <div className="flex items-center justify-between gap-3">
          <button onClick={() => scrollTo("hero")} className="group flex items-center gap-3 text-left">
            ...
            <span className="hidden min-w-0 sm:block"><span className={`block max-w-40 truncate text-sm font-bold ${primaryText}`}>{name}</span><span className="flex items-center gap-1 text-[10px] font-semibold text-emerald-600"><i className="h-1.5 w-1.5 rounded-full bg-emerald-500" />Available</span></span>
          </button>
          <nav className="hidden items-center gap-1 lg:flex">
            {items.map(([id, label]) => (
              <button key={id} onClick={() => scrollTo(id)} className={`relative rounded-lg px-3 py-2 text-xs font-bold transition-colors duration-200 ${activeSection === id ? primaryText : mutedText}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={toggleDarkMode} className={`rounded-xl p-2 transition ${controlText}`} aria-label="Toggle theme">{isDarkMode ? <Sun className="h-4 w-4 text-amber-400" /> : <Moon className="h-4 w-4" />}</button>
            <button onClick={() => setOpen(!open)} className={`rounded-xl p-2 lg:hidden ${primaryText}`} aria-label="Toggle navigation">{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
          </div>
        </div>
        <AnimatePresence>{open && <motion.nav initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className={`mt-3 grid gap-1 border-t pt-3 lg:hidden ${isDarkMode ? "border-white/10" : "border-slate-200"}`}>{items.map(([id, label]) => <button key={id} onClick={() => scrollTo(id)} className={`rounded-lg px-3 py-2 text-left text-sm font-semibold ${mutedText}`}>{label}</button>)}</motion.nav>}</AnimatePresence>
      </motion.div>
    </header>
  );
};
